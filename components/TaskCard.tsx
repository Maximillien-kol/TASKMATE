...
const TaskCard: React.FC<TaskCardProps> = ({
  ...
  return (
    <div className={`group relative bg-white rounded-2xl border transition-all hover:border-emerald-200 ${todo.completed
      ...
      <div className="p-4 sm:p-5 flex flex-col gap-3">
        {/* Row 1: Status, Category, Actions */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              onClick={() => onToggle(todo.id)}
              className={`shrink-0 w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${todo.completed ? 'bg-emerald-600 border-emerald-600 text-white' : 'border-slate-300 hover:border-emerald-400'
                }`}
            >
          ...
          <div className="flex items-center gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
            <button
              onClick={() => onEdit(todo)}
              title="Edit Task"
              className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all ${isOverdue ? 'text-rose-400 hover:text-rose-600 hover:bg-rose-100' : 'text-slate-400 hover:text-emerald-600 hover:bg-slate-50'
                }`}
            >
            ...
            <button
              ...
              title="AI Breakdown"
              className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all ${isOverdue ? 'text-rose-500 hover:bg-rose-100' : 'text-emerald-500 hover:bg-emerald-50'
        ...
        {/* Row 2: Title */}
        <div>
          <h3 className={`text-base font-bold leading-tight ${todo.completed ? 'line-through text-slate-400' : isOverdue ? 'text-rose-900' : 'text-slate-900'
            }`}>
            {todo.title}
          </h3>
        </div>
        ...
        {/* Row 3: Description */}
        {todo.description && (
          <p className={`text-sm leading-relaxed ${isOverdue ? 'text-rose-700/70' : 'text-slate-500'}`}>
            {todo.description}
          </p>
        )}

        {/* Row 4: Meta Tags (Priority, Time, Collaborators) */}
            ...
            <div className="flex -space-x-2 ml-1">
              {todo.collaborators.map((c, i) => (
                <div
                  key={i}
                  title={c}
                  className="w-5 h-5 rounded-full border border-white bg-emerald-100 flex items-center justify-center text-[8px] font-bold text-emerald-600 cursor-help"
                >
                  {getInitials(c)}
                </div>
              ))}
        ...
        {/* Row 5: Footer / Subtasks */}
        <div className={`flex items-center justify-between text-xs pt-2 border-t ${isOverdue ? 'border-rose-100' : 'border-slate-50'}`}>
          <span className={`font-medium ${isOverdue ? 'text-rose-500' : 'text-slate-400'}`}>
            {todo.subTasks.length > 0 ? `${todo.subTasks.filter(s => s.completed).length} / ${todo.subTasks.length} steps` : 'No steps'}
          </span>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handleAutoGenerate}
              disabled={isGenerating}
              className={`${isOverdue ? 'text-rose-600 hover:text-rose-700' : 'text-emerald-600 hover:text-emerald-700'} transition-colors flex items-center gap-1 font-bold disabled:opacity-50`}
            >
              {isGenerating ? (
            ...
            <button
              ...
              className="hover:text-slate-600 transition-colors flex items-center gap-1 px-2 py-1 rounded hover:bg-slate-50"
            >
              <span className="font-semibold">{isExpanded ? 'Collapse' : 'Expand'}</span>
          ...
          <div className="pt-2 space-y-4 animate-in slide-in-from-top-2 duration-200">
            ...
            <div className="space-y-2.5">
              ...
              {todo.subTasks.map(st => (
                <label key={st.id} className="flex items-start gap-3 cursor-pointer group/item">
                  <input
                    type="checkbox"
                    checked={st.completed}
                    onChange={() => onToggleSubtask(todo.id, st.id)}
                    className={`mt-0.5 rounded w-4 h-4 bg-transparent border-slate-300 ${isOverdue ? 'text-rose-600 focus:ring-rose-500' : 'text-emerald-600 focus:ring-emerald-500'}`}
                  />
                  <div className="flex-1 min-w-0">
                    <span className={`text-sm transition-colors break-words ${st.completed
                      ? 'line-through text-slate-400'
                      : isOverdue
                        ? 'text-rose-800 group-hover/item:text-rose-600'
                        : 'text-slate-600 group-hover/item:text-emerald-600'
                      }`}>
                      {st.title}
                    </span>
                  </div>
              ...
              <form onSubmit={handleAddSubtaskSubmit} className={`flex items-center gap-2 pt-2 border-t ${isOverdue ? 'border-rose-100' : 'border-slate-50'}`}>
                ...
                />
                {newSubtaskTitle && (
                  <button type="submit" className={`text-xs font-bold px-2 py-1 ${isOverdue ? 'text-rose-600 hover:text-rose-700' : 'text-emerald-600 hover:text-emerald-700'}`}>
                    Add
                  </button>
                )}
              </form>
            ...
            <div className={`space-y-2.5 pt-2 border-t ${isOverdue ? 'border-rose-100' : 'border-slate-50'}`}>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <form onSubmit={handleAddCollaborator} className="flex items-center gap-2">
                  ...
                  />
                  {newCollaborator && (
                    <button type="submit" className={`text-[10px] font-bold uppercase ${isOverdue ? 'text-rose-600' : 'text-emerald-600'}`}>
                      Invite
                    </button>
                  )}
                </form>
